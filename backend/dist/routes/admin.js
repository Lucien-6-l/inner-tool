...
router.post('/registrations/:id/resend', requireRole(Role.DEV, Role.ADMIN), async (req, res) => {
    const reg = await prisma.pendingRegistration.findUnique({ where: { id: req.params.id } });
    if (!reg) {
        res.status(404).json({ ok: false, error: '名单不存在' });
        return;
...
router.delete('/registrations/:id', requireRole(Role.DEV, Role.ADMIN), async (req, res) => {
    const reg = await prisma.pendingRegistration.findUnique({ where: { id: req.params.id } });
    if (!reg) {
        res.status(404).json({ ok: false, error: '名单不存在' });
        return;
