...
router.post('/add', async (req, res) => {
    ...
    const existing = await prisma.contact.findUnique({
        where: { ownerId_friendId: { ownerId: req.userId, friendId: userId } },
    });
    if (existing) {
    ...
    await prisma.contact.createMany({
        data: [
            { ownerId: req.userId, friendId: userId },
            { ownerId: userId, friendId: req.userId },
        ],
    });
    await ensureDirectConversation(req.userId, userId);
