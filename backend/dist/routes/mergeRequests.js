...
function requiredApprovals(total) {
    return Math.floor((total * 2) / 3) + 1;
}
async function canAccessTimeline(userId, role, timelineId) {
    if (role === 'DEV')
        return true;
    const tl = await prisma.timeline.findUnique({ where: { id: timelineId } });
    if (!tl)
        return false;
    if (tl.creatorId === userId)
        return true;
...
async function logActivity(timelineId, userId, actionType, branchId, detail) {
    await prisma.timelineActivity.create({
        data: { timelineId, userId, actionType, branchId, detail: detail ? JSON.stringify(detail) : null },
    });
}
// 创建汇入请求（从我的分支 → 主线）
router.post('/timelines/:timelineId/merge-requests', requireAuth, async (req, res) => {
    const { timelineId } = req.params;
    const { title, description } = req.body;
    if (!title?.trim()) {
        res.status(400).json({ ok: false, error: '请求标题必填' });
        return;
    }
    if (!await canAccessTimeline(req.userId, req.role, timelineId)) {
...
// 时间线下的汇入请求列表
router.get('/timelines/:timelineId/merge-requests', requireAuth, async (req, res) => {
    const { timelineId } = req.params;
    if (!await canAccessTimeline(req.userId, req.role, timelineId)) {
        res.status(403).json({ ok: false, error: '无权限' });
        return;
...
// 汇入请求详情
router.get('/merge-requests/:id', requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    if (!mr) {
        res.status(404).json({ ok: false, error: '请求不存在' });
        return;
    }
    if (!await canAccessTimeline(req.userId, req.role, mr.timelineId)) {
        res.status(403).json({ ok: false, error: '无权限' });
        return;
    }
    // 源分支文件（最新版本）
    ...
    for (const f of allFiles)
        if (!latestFiles.has(f.filename))
            latestFiles.set(f.filename, f);
    // 主线文件（用于标记冲突/覆盖）
    ...
    const mainFiles = mainBranch
        ...
        : [];
    const mainLatest = new Map();
    for (const f of mainFiles)
        if (!mainLatest.has(f.filename))
            mainLatest.set(f.filename, f.version);
...
router.post('/merge-requests/:id/vote', requireAuth, async (req, res) => {
    const { id } = req.params;
    const { vote, comment } = req.body;
    if (!['approve', 'reject', 'abstain'].includes(vote)) {
        res.status(400).json({ ok: false, error: '无效投票' });
        return;
    }
    if (vote === 'reject' && !comment?.trim()) {
        res.status(400).json({ ok: false, error: '驳回票必须填写具体问题所在' });
        return;
    }
    ...
    if (mr.status !== 'pending') {
        res.status(400).json({ ok: false, error: '该请求已结束，无法投票' });
        return;
    }
    ...
    if (!isParticipant && req.role !== 'DEV') {
        res.status(403).json({ ok: false, error: '仅项目参与者可投票' });
        return;
    }
    // 覆盖之前的投票
...
// 评论
router.post('/merge-requests/:id/comments', requireAuth, async (req, res) => {
    const { id } = req.params;
    const { content } = req.body;
    if (!content?.trim()) {
        res.status(400).json({ ok: false, error: '评论内容不能为空' });
        return;
    }
    const mr = await prisma.mergeRequest.findUnique({ where: { id } });
    if (!mr) {
...
router.post('/merge-requests/:id/merge', requireAuth, async (req, res) => {
    const { id } = req.params;
    const mr = await prisma.mergeRequest.findUnique({ where: { id } });
    if (!mr) {
        res.status(404).json({ ok: false, error: '请求不存在' });
    ...
    if (mr.status !== 'approved') {
        res.status(400).json({ ok: false, error: '请求未通过投票，无法汇入' });
        return;
    }
    if (!await canAccessTimeline(req.userId, req.role, mr.timelineId)) {
        res.status(403).json({ ok: false, error: '无权限' });
    ...
    const mainBranch = await prisma.timelineBranch.findFirst({ where: { timelineId: mr.timelineId, isMain: true } });
    if (!mainBranch) {
        res.status(500).json({ ok: false, error: '主线分支不存在' });
        return;
    }
    ...
    for (const f of sourceFiles)
        if (!latest.has(f.filename))
            latest.set(f.filename, f);
    let mergedCount = 0;
...
router.post('/merge-requests/:id/resubmit', requireAuth, async (req, res) => {
    const { id } = req.params;
    const { title, description } = req.body;
    const mr = await prisma.mergeRequest.findUnique({ where: { id } });
    if (!mr) {
    ...
    if (mr.createdById !== req.userId && req.role !== 'DEV') {
        res.status(403).json({ ok: false, error: '仅发起人可重新提交' });
        return;
    }
    if (mr.status !== 'rejected') {
        res.status(400).json({ ok: false, error: '仅被驳回的请求可重新提交' });
        return;
    }
