...
router.post('/', requireAuth, async (req, res) => {
    ...
    if (!name?.trim()) {
        res.status(400).json({ ok: false, error: '项目名称必填' });
        return;
    }
...
router.get('/:id', requireAuth, async (req, res) => {
    const { id } = req.params;
    if (!await canAccessTimeline(req.userId, req.role, id)) {
        res.status(403).json({ ok: false, error: '无权限查看' });
        return;
    }
    ...
    if (!tl) {
        res.status(404).json({ ok: false, error: '时间线不存在' });
        return;
    }
...
router.post('/:id/participants', requireAuth, async (req, res) => {
    ...
    if (tl.creatorId !== req.userId && req.role !== 'DEV') {
        res.status(403).json({ ok: false, error: '仅发起人可添加参与者' });
        return;
    }
    const added = [];
...
router.delete('/:id/participants/:userId', requireAuth, async (req, res) => {
    ...
    if (tl.creatorId !== req.userId && req.role !== 'DEV') {
        res.status(403).json({ ok: false, error: '仅发起人可移除参与者' });
        return;
    }
...
// ===== 分支与文件 =====
// 获取我的分支
router.get('/:id/my-branch', requireAuth, async (req, res) => {
    const { id } = req.params;
    if (!await canAccessTimeline(req.userId, req.role, id)) {
...
// 分支文件列表
router.get('/branches/:branchId/files', requireAuth, async (req, res) => {
    const { branchId } = req.params;
    const branch = await prisma.timelineBranch.findUnique({ where: { id: branchId } });
    if (!branch) {
        res.status(404).json({ ok: false, error: '分支不存在' });
        return;
    }
    if (!await canAccessTimeline(req.userId, req.role, branch.timelineId)) {
        res.status(403).json({ ok: false, error: '无权限' });
        return;
    }
    // 每个文件名取最新版本
...
// 上传文件到我的分支
router.post('/:id/my-branch/files', requireAuth, upload.single('file'), async (req, res) => {
    const { id } = req.params;
    if (!req.file) {
...
// 删除分支文件（仅分支拥有者）
router.delete('/branches/:branchId/files/:fileId', requireAuth, async (req, res) => {
    ...
    if (branch.ownerId !== req.userId && req.role !== 'DEV') {
        res.status(403).json({ ok: false, error: '仅分支拥有者可删除文件' });
        return;
    }
    const file = await prisma.timelineFile.findUnique({ where: { id: fileId } });
    await prisma.timelineFile.delete({ where: { id: fileId } });
    if (file)
        await logActivity(branch.timelineId, req.userId, 'file_deleted', branchId, { filename: file.filename, version: file.version });
    res.json({ ok: true });
});
// ===== 活动记录（版本链） =====
