 ...
 * 激活成功 / 存量同步时调用。
 */
export async function ensureDepartmentGroup(userId: string): Promise<string | null> {
  ...
  const convId = await prisma.$transaction(async (tx) => {
    ...
    if (missing.length > 0) {
      await tx.conversationMember.createMany({
        data: missing.map((m) => ({ conversationId: id as string, userId: m })),
      });
    }
    return id as string;
  });
 ...
 * 某个会话里、发给"我"且尚未读的消息数。
 */
export async function unreadCountOf(conversationId: string, userId: string): Promise<number> {
